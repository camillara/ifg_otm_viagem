from pulp import (
    ...
    LpBinary, LpContinuous, LpInteger, PULP_CBC_CMD, LpStatus
)
...
if __name__ == "__main__":
    # You must fill these with your data.
    V = ["A", "B", "C", "D"]
    origin, dest = "A", "D"
    ...
    # flight schedules (hours since 0)
    DEP = {
        ("A","B","f1"):  5, ("A","B","f2"): 30,
        ("B","C","f3"): 40,
        ("C","D","f4"): 70,
    ...
    C = {k: 1000 + 10*DEP[k] for k in DEP.keys()}  # dummy cost

    d_min = {i: 0.0 for i in V}
    d_max = {i: 10.0 for i in V}

    C_hotel = {i: 200.0 for i in V}
    C_food  = {i: 80.0  for i in V}
    C_transfer = {i: 50.0 for i in V}
    ...
    model = build_trip_milp_pulp(
        ...
        tau=24.0,
        D_total=5.0,
        TMAX=10.0,      # <= D_max (in hours of flight)
